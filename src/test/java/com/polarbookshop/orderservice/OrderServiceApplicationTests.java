...
class OrderServiceApplicationTests {
    ...
    void whenGetOrdersThenReturn() throws IOException {
        ...
        //bookClient의 예상 동작 정의
        given(bookClient.getBookByIsbn(bookIsbn)).willReturn(Mono.just(book));

        //Mock 객체인 bookClient를 이용해 ACCEPTED인 상태의 order를 DB에 삽입
        ...
        webTestClient.get().uri("/orders")
                ...
                .expectBodyList(Order.class).value(orders -> {
                    assertThat(orders.stream().filter(order -> order.bookIsbn().equals(bookIsbn)).findAny()).isNotEmpty(); //findAny는 Optional 객체로 반환
                });
    }

    //Mono<Book> 객체를 bookClient가 return하게 하여 post 요청 시 해당 book를 잘 return하는지 확인
    @Test
